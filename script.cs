...
async function emulateProgress(clientToken, promoId) {
    try {
        ...
        const data = await response.json();
        if (!response.ok) {
            console.error(`Emulate progress response error: ${JSON.stringify(data)}`);
            throw new Error(data.message || 'Failed to register event');
        }
        return data.hasCode;
    } catch (error) {
        console.error(`Error in emulateProgress: ${error.message}`);
        throw error;
    }
}
...
async function generateKey(clientToken, promoId) {
    try {
        ...
        const data = await response.json();
        if (!response.ok) {
            console.error(`Generate key response error: ${JSON.stringify(data)}`);
            throw new Error(data.message || 'Failed to generate key');
        }
        return data.promoCode;
    } catch (error) {
        console.error(`Error in generateKey: ${error.message}`);
        throw error;
    }
}
